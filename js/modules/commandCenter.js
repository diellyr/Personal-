  ...
  async function paint() {
    ...
    root.appendChild(h('div', { class: 'flex-between', style: 'margin-bottom:18px' }, [
      h('div', {}, [
        ...
        h('p', {}, new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })),
      ]),
      h('div', { class: 'tabs', style: 'border:none;margin:0' }, Object.keys(RANGE_DAYS).map((r) =>
        h('div', { class: `tab ${state.range === r ? 'active' : ''}`, onClick: () => { state.range = r; paint(); } }, r))),
    ]));
    ...
    const priorityScore = (t) => ({ CRITICAL: 4, HIGH: 3, MEDIUM: 2, LOW: 1 }[t.priority] || 2) + (t.dueDate && t.dueDate <= today ? 5 : 0);
    const top3 = [...openTasks].sort((a, b) => priorityScore(b) - priorityScore(a)).slice(0, 3);

    // ---- Foco do dia + Top priorities ----
    root.appendChild(sectionTitle('🎯 Foco do dia — 3 prioridades'));
    root.appendChild(h('div', { class: 'grid grid-3' }, top3.length ? top3.map((t) => h('div', { class: 'card' }, [
      h('div', { class: 'flex-between' }, [h('strong', {}, t.title), badge(t.priority, t.priority === 'CRITICAL' ? 'critical' : t.priority === 'HIGH' ? 'warning' : 'neutral')]),
      h('p', { style: 'margin-top:6px' }, `Módulo: ${t.module}${t.dueDate ? ' · Prazo: ' + fmtDate(t.dueDate) : ''}`),
    ])) : [h('div', { class: 'card muted' }, 'Nenhuma tarefa aberta. Crie tarefas em Tasks ou em qualquer módulo.')]));

    // ---- Indicators ----
    root.appendChild(sectionTitle('📊 Indicadores'));
    ...
    const balance = financeTx.reduce((acc, t) => acc + (t.data.type === 'INCOME' ? Number(t.data.amount || 0) : -Number(t.data.amount || 0)), 0);
    root.appendChild(h('div', { class: 'grid grid-4' }, [
      statTile('Tarefas abertas', openTasks.length, overdue.length ? `${overdue.length} atrasada(s)` : 'em dia'),
      statTile('Filhos cadastrados', familyChildren.length, 'Family'),
      statTile('Saldo financeiro', balance.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }), 'Finance'),
      statTile('Inglês (7 dias)', `${weekEnglishMin} min`, 'English'),
    ]));

    // ---- Agenda ----
    root.appendChild(sectionTitle(`📅 Agenda — ${state.range}`));
    const agendaItems = [
      ...openTasks.filter((t) => t.dueDate && inRange(t.dueDate)).map((t) => ({ date: t.dueDate, title: t.title, source: 'Task', module: t.module })),
      ...churchAgenda.filter((a) => inRange(a.data.date)).map((a) => ({ date: a.data.date, title: a.data.title, source: 'Church', module: 'church' })),
      ...jobInterviews.filter((i) => inRange(i.data.date)).map((i) => ({ date: i.data.date, title: `Entrevista — ${i.data.company || ''}`, source: 'Job Hunter', module: 'jobs' })),
      ...trips.filter((tr) => inRange(tr.data.startDate)).map((tr) => ({ date: tr.data.startDate, title: `Viagem — ${tr.data.destination}`, source: 'Travel', module: 'hobbies-travel' })),
    ].sort((a, b) => (a.date < b.date ? -1 : 1));
    root.appendChild(agendaItems.length
      ? h('div', { class: 'table-wrap' }, h('table', { class: 'data-table' }, [
          h('thead', {}, h('tr', {}, [h('th', {}, 'Data'), h('th', {}, 'Item'), h('th', {}, 'Origem')])),
          h('tbody', {}, agendaItems.map((it) => h('tr', {}, [h('td', {}, fmtDate(it.date)), h('td', {}, it.title), h('td', {}, badge(it.source, 'neutral'))]))),
        ]))
      : h('div', { class: 'card muted' }, 'Nada agendado neste período.'));
    ...
    root.appendChild(sectionTitle('🤖 AI Chief of Staff — principais insights', h('button', { class: 'link-btn', onClick: () => navigate('/ai-insights') }, 'Ver todos →')));
